...
\todo{I think the key is that future iterations can and should change
  inferred output, but must not change programmer-provided input.  So these
  must be input differently to the inference tool, so that it can treat
  them differently.  Maybe organizing the section around that will make it
  shorter and clearer.}
...
\todo{Try to avoid writing this as a narrative of things we did in the
  order we did them.  Instead,
  lay out alternatives and the pluses and minuses of each.}
...
described above.
\todo{This deserves a little more explanation (even as other parts may be
  shorter).  This inference-specific side file mechanism uses the same
  syntax but different syntax than the existing side file mechanism.  The
  implementation section can point out that we actually designed an new
  mechanism (ajava files), because the existing mechanisms were not
  adequate for inference.}
