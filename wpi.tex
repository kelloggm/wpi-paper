...
%% article.
\begin{abstract}
  Type inference is useful: it makes it easier for programmers to use
  a type system by inferring type annotations, which programmers would
  ...
  Pluggable type systems extend a host type system with type qualifiers related
  to some property of interest.
  Unfortunately, traditional type inference
  techniques for some host language do not scale well to pluggable type
  systems: for each new pluggable type system, the type inference algorithm
  needs to be extended to build and then solve constraints relevant to the
  ...
  our new technique is that pluggable type systems are \emph{flow-sensitive}
  ...
  project, which includes a variety of specialized pluggable typecheckers
  for Java. We used our inference algorithm with \todo{some number of} distinct
