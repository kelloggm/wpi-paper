...
11. Measure the percentage of hand-written annotations that WPI inferred
    ...
    i. run the InferredAnnosCounter, with the first argument being the path of the human-written code, and other arguments being the path of the .ajava files produced by the WPI. The program assumes that a formatter has been applied, so it is important to do so before passing the files as input. The way to run InferredAnnosCounter is like this: ``` gradle run --args = "(a path to the human-written file) (one or more paths to the computer-generated files)" ```
