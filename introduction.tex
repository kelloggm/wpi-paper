...
Uber~\todo{citation}, Google~\todo{citation}, and others.

A problem with pluggable types %(and other type systems, too!)
...
\todo{It would be great here to have some evidence that users believe this is a problem.
  I think we know that's the case, but maybe we could interview e.g., Martin Schaef about
  why Amazon didn't adopt the CF as much as we'd have liked and include a quote about
  annotations here?}

% I considered text like this at this point, but I think its orthogonal to my main point.
%% Further, most pluggable
%% typecheckers are not perfectly precise: that is, the typechecker may reject
%% programs that are correct at runtime\footnote{Note again that this is also
%% a problem for ``traditional'' type systems!}. When annotating a legacy program,
%% a programmer may discover code that, while correct, cannot be proven so
%% by the typechecker.
